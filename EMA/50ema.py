...
output_base_dir = r'Technicals\50EMA'

# Function to calculate the 50-day EMA
...
def process_csv(csv_file):
    ...
    # Ensure there is enough data to calculate the 50-day EMA
    if len(df) < 50:
        return f"Not enough data for {csv_file}. File skipped."
    
    # Calculate the 50-day EMA
    df_with_ema = calculate_ema(df, period=50)
    ...
    # Get the latest 50-day EMA value
    ...
    if (stock_current_price - a) < latest_ema < (stock_current_price):
        ...
        return f"Condition met for {stock_name}! 50-day EMA calculation completed and saved to '{output_file}'"
    else:
