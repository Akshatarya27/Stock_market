...
output_base_dir = r'Technicals\200EMA'
...
def process_csv(csv_file):
    ...
    # Ensure there is enough data to calculate the 200-day EMA
    if len(df) < 200:
        return f"Not enough data for {csv_file}. File skipped."
    
    # Calculate the 200-day EMA
    df_with_ema = calculate_ema(df, period=200)
    ...
    a = 0.10 * stock_current_price
    
    # Get the latest 200-day EMA value
    ...
    if (stock_current_price - a) < latest_ema < (stock_current_price):
        ...
        return f"Condition met for {stock_name}! 200-day EMA calculation completed and saved to '{output_file}'"
    else:
